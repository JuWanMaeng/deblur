...
class Blur_generative_model(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(Blur_generative_model, self).__init__(opt)
    ...
    def feed_data(self, data, is_val=False):
        ...
        # this model is for kernel_generative model... 2023/07/25
        # self.gt * blur kernel - self.lq #
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        preds = self.net_g(self.lq)   ### preds= [blur_kernel, None] ###
        if not isinstance(preds, list):
            preds = [preds]

        self.output = preds[-1] ### no use in generative model 
        ...
        if self.cri_reblur:
            l_reblur = 0.
            fake_lq = self.degradation_model(self.gt, preds[0])  ### 원본 이미지에 blur kernel을 처리해줌
            l_reblur += self.cri_reblur(fake_lq, self.lq)
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if isinstance(pred, list):
                    pred, kernel_pred = pred[0], pred[1]
                    # kernel_pred.shape: [B, kernel_size * kernel_size, 256,256]
                fake_lq=self.degradation_model(self.gt,pred)
                outs.append(fake_lq.detach().cpu())
                kernel_outs.append(kernel_pred.detach().cpu())
                i = j

            # self.output = torch.cat(outs, dim=0)  # tensor[1, 3, H, W]
            self.fake_lq = torch.cat(outs, dim=0)
            self.kernel = torch.cat(kernel_outs, dim=0)
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        with_loss=self.opt['val'].get('loss') is not None
        val_opt=self.opt['val']

        if with_loss:  # 완성된 생성모델의 지표를 기록하기 위해
            reblur_type = val_opt['loss'].pop('type')
            cri_reblur_cls = getattr(loss_module, reblur_type)
            self.cri_reblur = cri_reblur_cls(**val_opt['loss']).to(
                self.device)
            self.reblur_weight = val_opt['loss'].pop('loss_weight')
        else:
            self.cri_reblur = None
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # self.test_crop9()
            self.test() # -> make self.fake_blur batch
           ...
           #########################2023/07/29####################################
            if self.cri_reblur:
                self.fake_lq=self.fake_lq.to(self.device)              
                l_reblur = self.cri_reblur(self.fake_lq, self.lq)
                # print(l_reblur)
            ...
            ############## 2023/07/26 #################
            visuals = self.get_current_visuals()
            fake_blur=visuals['fake_blur']
            sr_img = tensor2img([visuals['fake_lq']], rgb2bgr=rgb2bgr)
            ...
            # 합성된 블러 커널
            np.save(osp.join(f'/workspace/data/UFP_numpy/{img_name}_kernel.npy'),fake_blur)
            ...
            if with_metrics:
                ...
                if use_image:
                    ...
                    for name, opt_ in opt_metric.items():
                        ...
                        self.metric_results[name] += getattr(
                            metric_module, metric_type)(visuals['fake_lr'], visuals['lq'], **opt_)                        
    ...
    def get_current_visuals(self):
        ...
        out_dict = OrderedDict()
        out_dict['lq'] = self.lq.detach().cpu()
        out_dict['fake_lq'] = self.fake_lq.detach().cpu()
        out_dict['fake_blur'] = self.kernel.detach().cpu()
        if hasattr(self, 'gt'):
            out_dict['gt'] = self.gt.detach().cpu()
        return out_dict
