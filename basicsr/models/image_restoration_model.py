...
from basicsr.utils.sv_blur import BatchBlur_SV,BatchBlur_SV_NAF
...
class ImageRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if rank == 0:
            wandb.init(project='NAF_KAM')
            wandb.run.name = 'GoPro'
            # wandb.init(settings=wandb.Settings(code_dir="~/Downloads/guided-diffusion"))
            # wandb.run.log_code( ".",
    ...
    def dist_init_training_settings(self):
        self.net_g.train()
        ...
        if train_opt.get('perceptual_opt'):
            ...
            self.cri_perceptual = cri_perceptual_cls(
                **train_opt['perceptual_opt']).to(self.net_g.device)
        else:
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        rank,_ = get_dist_info()
        if current_iter % 20 ==0 and rank == 0 :
            wandb.log({'psnr loss': loss_dict['l_pix'].item(), 'iter':current_iter})
            wandb.log({'reblur loss': loss_dict['l_reblur'].item(), 'iter':current_iter})

    def optimize_parameters_origin(self, current_iter, tb_logger):
        self.optimizer_g.zero_grad()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                pred = self.net_g(self.lq[i:j], self.kernel)
        ...
        ###### 2023/08/01 ###### 모든 test set에 대한 blur kernel을 추정해보기 위한 코드
        # self.net_g.eval()
        # with torch.no_grad():
        #     n = len(self.lq)    # tensor[1, 3, H, W]
        #     outs = []
        ...
        #         if isinstance(pred, list):
        #             pred, kernel_pred = pred[-1], pred[1]
        #         outs.append(pred.detach().cpu())
        #         kernel_outs.append(kernel_pred.detach().cpu())
        #         i = j

        #     self.output = torch.cat(outs, dim=0)  # tensor[1, 3, H, W]
        #     self.kernel = torch.cat(kernel_outs, dim=0) # tensor [1, 19*19, H, W]
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            ### test set에 대한 blur kernel 확인하기 위한 코드 ###
            # np.save(osp.join(f'hdd/blur_kernels/{img_name}_kernel.npy'),self.kernel)
            # del self.kernel
            ...
            if save_img:
                if sr_img.shape[2] == 6:
                    ...
                    # visual_dir = osp.join('hdd/results', dataset_name)
                    ...
                    if self.opt['is_train']:
                        ...
                        save_img_path = osp.join(
                            self.opt['val'].get('save_img_path'), dataset_name,
                            f'{img_name}.png')
...
### 2023/07/31 ###
    def predict(self,img,path):
        ...
        # Change the layout from HWC to CHW
        ...
        with torch.no_grad():
            pred=self.net_g(image_tensor)
            if isinstance(pred, list):
                pred = pred[-1]
        result_tensor=pred.detach().cpu()
        result_img=tensor2img(result_tensor)
        path=path.split('_')[0]
        cv2.imwrite(f'{path}_result_img.png',result_img)

        self.net_g.train()


class DegradationModel_NAF(nn.Module):
    def __init__(self, kernel_size=19):
        super(DegradationModel_NAF, self).__init__()
        self.blur_layer = BatchBlur_SV_NAF(l=kernel_size, padmode='zero')
