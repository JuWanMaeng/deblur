...
class NAFBlock(TimestepBlock):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0., time_embed_dim = 256):
        super().__init__()
        ...
        self.emb_channels = time_embed_dim
...
class NAFBlock_kernel(TimestepBlock):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0., kernel_size=19, time_embed_dim = 256):
    ...
    def forward(self, inp, kernel, emb):
        x = inp
...
class NAFNet_KAMT(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], kernel_size=19):
        ...
        self.time_embed = nn.Sequential(
            ...
            nn.SiLU(),
            linear(width * 4, width * 4),
        )

        chan = width
        for num in enc_blk_nums:
            if num==1:
                self.encoders.append(TimestepEmbedSequential(*[NAFBlock_kernel(chan, kernel_size=kernel_size) for _ in range(num)]))
            else:
                self.encoders.append(TimestepEmbedSequential(*[NAFBlock(chan) for _ in range(num)]))
            self.downs.append(
    ...
    def forward(self, inp, kernel, timesteps):
        B, C, H, W = inp.shape
        inp = self.check_image_size(inp)

        emb = self.time_embed(timestep_embedding(timesteps, self.width))  # timestep embeding
        ...
        for encoder, down, kernel_down in zip(self.encoders, self.downs, self.kernel_down):
            if len(encoder) == 1:
                x = encoder[0](x, kernel, emb)  # kernel_attention(KAM) -> Block 과정
...
class NAFNet_KAMT_Local(Local_Base, NAFNet_KAMT):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFNet_KAMT.__init__(self, *args, **kwargs)
