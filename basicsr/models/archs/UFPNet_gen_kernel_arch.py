...
class UFPNet_gen_kernel_arch(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], kernel_size=19):
        ...
        self.kernel_extra.load_state_dict(torch.load('weights/kernel_extra.pth'))
        self.flow.load_state_dict(torch.load('weights/flow.pth'))
    ...
    def forward(self, inp):
        ...
        # kernel_code: kernel_mean, kernel_var=kernel_val
        ...
        kernel_code = (kernel_code - torch.mean(kernel_code, dim=[2, 3], keepdim=True)) / torch.std(kernel_code,
                                                                                                    dim=[2, 3],
                                                                                                    keepdim=True)
        # code uncertainty 공간적으로 다양한 블러 커널의
        sigma = kernel_var
        ...
        kernel = generate_k(self.flow, kernel_code_uncertain.reshape(kernel_code.shape[0]*kernel_code.shape[1], -1)) # generative model
        kernel = kernel.reshape(kernel_code.shape[0], kernel_code.shape[1], self.kernel_size, self.kernel_size)
        kernel_blur = kernel
        # 모양: [B, 커널_크기, 커널_크기, 256, 256] -> return 해야하는 값

        kernel = kernel.permute(0, 2, 3, 1).reshape(B, self.kernel_size*self.kernel_size, inp.shape[2], inp.shape[3])
        # shape: 형태: [B, kernel_size * kernel_size, 256, 256]
        ...
        return [kernel_blur,kernel, None]
...
class UFPNet_gen_kernel_arch_Local(Local_Base, UFPNet_gen_kernel_arch):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        UFPNet_gen_kernel_arch.__init__(self, *args, **kwargs)
