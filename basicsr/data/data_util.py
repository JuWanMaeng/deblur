...
def paired_paths_from_meta_info_file(folders, keys, meta_info_file,
    ...
    return paths   # {[gt_path: path, lq_path: path], [],[],[] ...}
...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    input_paths = list(scandir(input_folder))   # imgs names
    gt_paths = list(scandir(gt_folder))         # imgs names '000001.png'
    ...
    for idx in range(len(gt_paths)):
        ...
        basename, ext = osp.splitext(osp.basename(gt_path))  # basename: 000135, ext: .png
        input_path = input_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(input_path))
        input_name = f'{filename_tmpl.format(basename)}{ext_input}' # 대응되는 이미지 찾는 과정
        input_path = osp.join(input_folder, input_name)
        if  input_name in input_paths:
            gt_path = osp.join(gt_folder, gt_path)  # full address
            paths.append(
                dict([(f'{input_key}_path', input_path),
                    (f'{gt_key}_path', gt_path)]))
            ...
            gt_path = osp.join(gt_folder, gt_path)  # full address
            input_path = osp.join(input_folder, 'blur'+input_name[2:])
            paths.append(
                dict([(f'{input_key}_path', input_path),
                    (f'{gt_key}_path', gt_path)]))
...
def three_paired_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    input_paths = list(scandir(input_folder))   # imgs names
    gt_paths = list(scandir(gt_folder))         # imgs names '000001.png'
    kernel_paths = list(scandir(kernel_folder))
    ...
    for idx in range(len(gt_paths)):
        ...
        basename_input, ext_input = osp.splitext(osp.basename(kernel_path))
        kernel_name = f'{filename_tmpl.format(basename)}{ext_input}' # 대응되는 이미지 찾는 과정
        kernel_path = osp.join(kernel_folder, kernel_name)
