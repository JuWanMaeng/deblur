...
from basicsr.data.paired_image_kernel_dataset import load_pair_data_NAFNet_dist
...
from basicsr.utils import (MessageLogger, check_resume, get_env_info,
                           ...
                           set_random_seed)
from basicsr.utils.dist_util import get_dist_info, init_dist, setup_dist
...
def parse_options(is_train=True):
    ...
    parser.add_argument(
    '-opt', type=str, default='options/train/NAFNet_KAM-GoPro.yml', help='Path to option YAML file.')
    parser.add_argument(
        '--launcher',
    ...
    opt['rank'], opt['world_size'] = get_dist_info()  # 분산학습 환경 조회
...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            dataset_enlarge_ratio = dataset_opt.get('dataset_enlarge_ratio', 1) # 찾을수 없으면 1로 반환

            train_loader = load_pair_data_NAFNet_dist(dataset_opt)
            ...
            num_iter_per_epoch = math.ceil(
                len(train_loader.dataset) * dataset_enlarge_ratio /
...
def main():
    ...
    # automatic resume ..
    state_folder_path = 'UFPDeblur/experiments/{}/training_states/'.format(opt['name'])
    import os
    try:
    ...
    while current_iter <= total_iters:
        ...
        result_code = model.optimize_parameters(current_iter, tb_logger)  # train
        ...
        if current_iter % opt['logger']['save_checkpoint_freq'] == 0:
            ...
            # # validation
            # if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0 or current_iter == 2000):
